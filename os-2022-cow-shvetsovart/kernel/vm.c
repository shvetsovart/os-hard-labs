...
int uvmcopy_page(pagetable_t pagetable, uint64 va) {
  ...
  flags = PTE_FLAGS(*pte) & ~PTE_RSW;

  if ((mem = kalloc())) {
    memmove(mem, (char *)PTE2PA(*pte), PGSIZE);
    kfree((char *)PTE2PA(*pte));
  } else {
    printf("uvmcopy_page: kalloc failed\n");
    return -1;
  }
...
pte_t *walkuser(pagetable_t pagetable, uint64 va) {
  pte_t *pte;
...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len) {
  ...
  while (len > 0) {
    ...
    pte = walkuser(pagetable, va0);

    if (pte == 0) return -1;

    if ((*pte & PTE_RSW) != 0) {
      if (uvmcopy_page(pagetable, va0)) return -1;
    }
