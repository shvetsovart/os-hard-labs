...
int xor_bit_isset(char *array, int index) {
  index >>= 1;
...
void xor_bit_set(char *array, int index) {
  index >>= 1;
  char b = array[index / 8];
  char m = (1 << (index % 8));
  array[index / 8] = (b ^ m);
...
void *bd_malloc(uint64 nbytes) {
  ...
  xor_bit_set(bd_sizes[k].alloc, blk_index(k, p));
  ...
  for (; k > fk; k--) {
    ...
    xor_bit_set(bd_sizes[k - 1].alloc, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    xor_bit_set(bd_sizes[k].alloc, bi);
    if (xor_bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
int bd_initfree_pair(int k, int bi, void *start, void *end) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (xor_bit_isset(bd_sizes[k].alloc, bi)) {  // !
    // one of the pair is free
    free = BLK_SIZE(k);
    if (start <= addr(k, buddy) && addr(k, buddy) < end)  // !
